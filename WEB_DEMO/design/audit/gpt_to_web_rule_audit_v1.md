...
| 地图生成 | 主路径、分支、河流、断桥、雾门、营地、资源和特殊点 | 必须继承节奏 | v0.2 基础骨架 | 完整节点后续迁移 |
...
| 辉石 | 建设成本、诱饵、失败缓冲三重用途 | 必须继承 | v0.2 部分实现 | 已实现采集/放置/生命周期，夜晚用途后续迁移 |
...
| HUD | 显示天数、阶段、辉石、人口、单位、夜晚黑影计数 | 必须继承信息价值 | v0.2 基础已实现 | 只显示当前版本需要的信息 |
...
- GPT_DEMO 中辉石靠 `pickup()` 自动拾取；本任务卡验收要求“面向辉石按 Space 确认拾取”，因此 v0.2 在 `InteractionSystem` 中加入了辉石 Space 采集。后续需要确认最终体验是自动拾取、Space 拾取，还是两者并存。
